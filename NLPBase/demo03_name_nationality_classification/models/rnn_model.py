...
class My_RNN(nn.Module):
    """
    名前（文字列）を分類するための標準的なRNNカスタムモデルクラスです。
    """
    ...
    def forward(self, input_tensor: torch.Tensor, hidden: torch.Tensor) -> Tuple[torch.Tensor, torch.Tensor]:
        ...
        Args:
            ...
            hidden (torch.Tensor): 初期隠れ状態テンソル

        Returns:
            Tuple[torch.Tensor, torch.Tensor]: 出力結果と更新された隠れ状態
        """
        # input_tensorの形状を [シーケンス長, バッチサイズ(1), 入力次元数] に変換
        input_tensor = input_tensor.unsqueeze(1)

        # RNNレイヤーへ入力
        output, hn = self.rnn(input_tensor, hidden)
...
if __name__ == '__main__':
    ...
    for tensor_x, tensor_y in my_dataloader:
        ...
        # 初期隠れ状態の生成と順伝播の実行
        initial_hidden = my_rnn.init_hidden()
        output_tensor, hidden_out = my_rnn(sample_input, initial_hidden)
