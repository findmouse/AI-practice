...
class My_GRU(nn.Module):
    """
    名前（文字列）を分類するためのGRUカスタムモデルクラスです。
    """
    ...
    def forward(self, input_tensor: torch.Tensor, hidden: torch.Tensor) -> Tuple[torch.Tensor, torch.Tensor]:
        ...
        # GRUレイヤーへ入力
        output, hn = self.gru(input_tensor, hidden)
...
if __name__ == '__main__':
    ...
    for tensor_x, tensor_y in my_dataloader:
        ...
        # 初期隠れ状態の生成と順伝播の実行
        initial_hidden = my_gru.init_hidden()
        output_tensor, hidden_out = my_gru(sample_input, initial_hidden)
