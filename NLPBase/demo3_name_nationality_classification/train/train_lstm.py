...
def train_lstm() -> Tuple[List[float], int, List[float]]:
    """
    LSTMモデルの学習を行い、損失の履歴、総実行時間、精度の履歴を返します。
    ...
    my_optim = optim.Adam(my_lstm.parameters(), lr=LR)
    ...
    for epoch_idx in range(EPOCHS):
        for i, (x, y) in enumerate(tqdm(my_dataloader, desc=f"Epoch {epoch_idx + 1}/{EPOCHS}")):
            ...
            output, hn, cn = my_lstm(input_tensor=x[0], hidden=h0, cell=c0)
        ...
        model_path = os.path.join(model_dir, f'my_lstm_model_{epoch_idx + 1}.bin')
        torch.save(my_lstm.state_dict(), model_path)
    ...
    results_path = os.path.join(results_dir, 'ai_lstm_train_result.json')
