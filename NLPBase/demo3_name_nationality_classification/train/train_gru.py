...
def train_gru() -> Tuple[List[float], int, List[float]]:
    """
    GRUモデルの学習を行い、損失の履歴、総実行時間、精度の履歴を返します。
    ...
    my_optim = optim.Adam(my_gru.parameters(), lr=LR)
    ...
    for epoch_idx in range(EPOCHS):
        ...
        for i, (x, y) in enumerate(tqdm(my_dataloader, desc=f"Epoch {epoch_idx + 1}/{EPOCHS}")):
            ...
            output, hn = my_gru(input_tensor=x[0], hidden=my_gru.init_hidden())
        ...
        model_path = os.path.join(model_dir, f'my_gru_model_{epoch_idx + 1}.bin')
        torch.save(my_gru.state_dict(), model_path)
    ...
    results_path = os.path.join(results_dir, 'ai_gru_train_result.json')
